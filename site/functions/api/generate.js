...
export async function onRequestPost({ request, env }) {
  ...
  const job = {
    ...
    seriesMode: body.seriesMode || "series",
    episodeDuration: Number(body.episodeDuration || 60),
    episodeCount: Number(body.episodeCount || 1),
    activeEpisode: body.activeEpisode || "EP01",
    storyboardCount: Number(body.storyboardCount || segmentCount),
    storyboardCountPerEpisode: Number(body.storyboardCountPerEpisode || 1),
    textModel,
    imageModel,
    videoModel,
    ...
    productImageName: body.productImageName || "",
    hasProductImageReference: Boolean(body.productImagePreview),
  ...
  const llmResult = await callProvider(env, textModel, {
    ...
    storyboardRule: `一张故事板必须对应一条 ${videoConfig.duration} 秒视频。按剧情需要生成故事板，不要固定数量。`,
    script: body.script || "",
    productBrief: body.productBrief || "",
    productImageName: body.productImageName || "",
    productImageReferenceNote: body.productImagePreview ? "用户上传了产品图参考，生成资产和故事板时必须锁定商品外观、包装、颜色和Logo。" : "",
    brandBrief: body.brandBrief || "",
    placementRules: body.placementRules || "",
    seriesPremise: body.seriesPremise || "",
    output_format: "json"
  ...
  const imageResult = await callProvider(env, imageModel, {
    ...
    job,
    storyboard: body.storyboard || null,
    storyboardRule: `一张故事板对应一条 ${videoConfig.duration} 秒视频。`,
    styleRoute: body.styleRoute,
    productImageName: body.productImageName || "",
    productImageReferenceNote: body.productImagePreview ? "用户上传了产品图参考，商品外观必须保持一致。" : "",
